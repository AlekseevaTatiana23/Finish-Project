...
Для просмотра существующий веток необходимо использовать команду **`git branch`**. Для отображения веток и слияний в графическом виде используется команда **`git log --graph`**.